...
public class FibHeap {
    ...
    public FibNode min;        // 最小节点(某个最小堆的根节点)
    public FibNode[]   pos;//位置
    ...
    public class FibNode {
        ...
        public FibNode(int index,int weight) {
            this.index    = index;
            this.weight    = weight;
            this.degree = 0;
            this.marked = false;
    ...
    public FibHeap(int point){
        this.keyNum = 0;
        this.min = null;
        this.pos = new FibNode[point];
    }
...
//    public void insert2(int index,int weight){
//        FibNode node;
//
//        node = new FibNode(index,weight);
//        pos[index]=node;
//        if (node == null)
//            return ;
//
...
//    }
    ...
    public void union(FibHeap other) {
        ...
        if((this.min) == null) {                // this无"最小节点"
            ...
            if (this.min.weight > other.min.weight)
                this.min = other.min;
            this.keyNum += other.keyNum;
            other = null;;
        }
    ...
    private void consolidate() {
        ...
        while (min != null) {
            ...
            while (cons[d] != null) {
                ...
                if (x.weight > y.weight) {    // 保证x的键值比y小
                    FibNode tmp = x;
                    x = y;
                    y = tmp;
                }
     ...
     */
    private void decrease(FibNode node, int newWeight) {
     ...
     */
    public void update(FibNode node, int newWeight) {
//        if(newWeight < node.weight)
        decrease(node, newWeight);
//        else if(newWeight > node.weight)
//            increase(node, newWeight);
//        else
//            System.out.printf("No need to update!!!\n");
    }

    public void update(int index, int newkey) {
        FibNode node;
        node = get(index);
        if (node!=null)
            update(node, newkey);
    ...
    public void newUpdate(int index,int newkey){
        FibNode node = get(index);
    }

    public FibNode get(int index) {
        if (min==null)
            return null;
        return pos[index];
    ...
    private FibNode get(FibNode root, int index) {
        FibNode t = root;    // 临时节点
    ...
    private void print(FibNode node, FibNode prev, int direction) {
        ...
        do {
            if (direction == 1)
                System.out.printf("%8d(%d)-%d is %2d's child\n", node.index, node.degree,node.weight, prev.index);
            else
                System.out.printf("%8d(%d)-%d is %2d's next\n", node.index, node.degree,node.weight, prev.index);
    ...
    public void print() {
        ...
        do {
            i++;
            System.out.printf("%2d. %4d(%d)-%d is root\n", i, p.index, p.degree,p.weight);
